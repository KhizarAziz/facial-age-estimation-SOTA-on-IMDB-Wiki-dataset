...
def init_dataset_meta_csv():
  image_dir = dataset_base_path.joinpath('images')

  dataset_meta_list = []
  for file_path in image_dir.glob('*.JPG'):
    img_name = str(file_path).split('/')[-1]
    temp_name = img_name.split('.')[0]
    age = float(temp_name[4:6])

    dataset_meta_list.append([file_path,age])

  # make a dataframe dataset
  dataset_meta_df = pd.DataFrame(dataset_meta_list, columns=['full_path','age'])
...
def loadData_preprocessData_and_makeDataFrame():
  ...
  for index,series in meta_dataframe.iterrows():
    ...
    properties_list.append([image_path,series.age,image_buffer,face_rect_box_serialized,face_landmarks_serialized])
    if index%500 == 0:
      print(index,'images added processed')
  processed_dataset_df = pd.DataFrame(properties_list,columns=['image_path','age','image','org_box','landmarks'])
...
def rectify_data():
    ...
    print(Dataset_DF.groupby(["age", ""]).agg(["count"]))
...
Dataset_DF = pd.DataFrame(columns=["age", "", "image", "org_box", "trible_box", "landmarks", "roll", "yaw", "pitch"])

# define all parameters here
dataset_base_path = Path('/content/RP/dataset/FGNET/')
dataset_name = 'FGNET' # different dataset name means different sequence for loading etc
